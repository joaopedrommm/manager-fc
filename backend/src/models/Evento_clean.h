...
//   Evento é a classe base abstrata. Cada tipo de lance (gol, cartão, intervalo) é uma classe filha que herda os atributos comuns (minuto, time) e 
//   reimplementa getDescricao() com sua própria narrativa.

//   Em Simulacao.cpp, todos os eventos são guardados como Evento* numa LinkedList. Ao exibir, chamamos getDescricao() sem saber o tipo real — o C++ escolhe a versão certa automaticamente.
...
public:
    ...
    // MÉTODO VIRTUAL: cada subclasse define sua própria descrição.
...
public:
    ...
    // As frases ficam como static local dentro do método — solução correta para arrays constantes em arquivos de cabeçalho.
...
// Além da narração, impacta o jogo: o time fica com um jogador a menos, e as forças por setor são recalculadas automaticamente no ContextoPartida.
